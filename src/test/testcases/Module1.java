...
public class Module1 extends UserLibrary
{
	...
	public static void TC11_validateCancelLeave()
	{
		boolean stepstatus;
		//Step 1: Login to HRM as a Team member
		stepstatus = Login(URL, UserName , Password);		
		if(stepstatus)	
			LogEvent("pass","Login successful");
		else
			LogEvent("fail","Login not successful");
		//Step 2: Navigate t oMyleave page
		stepstatus = NavigateMyLeave();
		if(stepstatus)	
			LogEvent("pass","My Leave page opened");
		else
			LogEvent("fail","My Leave page not opened");
		
		//Step 3: Verify user is able cancel leave		
		
				
		
		
		//Step 4: Logout from HRM		
		Logout();		
	...
	public static void TC1()
	{
		...
		//Step 1: luanch HRM login page
		status = OpenUrl(URL);
		if(status)
			LogEventWithScreeshot("info", "App launched");
		else
			LogEventWithScreeshot("fail", "App not launched");
	
		//Step 2: Entere user name
		status = SetText("//*[@id='txtUsername']", "user01");
		if(status)
			LogEventWithScreeshot("pass", "User name entred");
		else
			LogEventWithScreeshot("fail", "user name not entred");
		
		//Step 3: Entere password
		status = SetText("//*[@id='txtPassword']", "pass1234");
		if(status)
			LogEventWithScreeshot("pass", "Password entred");
		else
			LogEventWithScreeshot("fail", "Password not entred");
